...
//Epsilon taken for correctness checking
const static double EPSILON = 1e-9;

/*---------------------------------------------------------------
  Baseline scalar stencil kernel (given)
----------------------------------------------------------------*/
...
void stencil_3d_7pt_unrolled(const double* curr, double* next) {
    for (int i = 1; i < (int)NX - 1; ++i) {
        for (int j = 1; j < (int)NY - 1; ++j) {
            ...
            // bdry conditions check for no out of bound accesess
...
#pragma omp parallel for collapse(2) schedule(static)
    for (int i = 1; i < (int)NX - 1; ++i) {
        for (int j = 1; j < (int)NY - 1; ++j) {
            int k = 1;
            // boundary condition to ensure no out of bound accesses
            for (; k <= (int(NZ) - 2) - (UNROLL_FACTOR - 1); k += UNROLL_FACTOR) {
#pragma unroll(UNROLL_FACTOR)
                for (int u = 0; u < UNROLL_FACTOR; ++u) {
                    int kk = k + u;
            ...
            // tail loop: handle remaining k values if any
            for (; k < (int)NZ - 1; ++k) {
                double neighbors_sum =
                    curr[(i + 1) * NY * NZ + j * NZ + k] +
                    curr[(i - 1) * NY * NZ + j * NZ + k] +
...
/*---------------------------------------------------------------
  Initialisation and checksum helpers
----------------------------------------------------------------*/
...
int main(int argc, char* argv[]) {
    ...
    assert(std::fabs(base_center - unroll_center) < EPSILON &&
           "Center value mismatch between baseline and opt versions!");
    assert(std::fabs(base_sum - unroll_sum) < EPSILON &&
           "Total sum mismatch between baseline and opt versions!");
