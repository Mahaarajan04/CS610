...
const uint64_t NX = SZ+2; // 64 interior points + 2 boundary points
const uint64_t NY = SZ+2;
const uint64_t NZ = SZ+2;
...
#pragma unroll(UNROLL_FACTOR)
            ...
            // tail loop: handle remaining k values
            for (; k < (int)NZ - 1; ++k) {
                double neighbors_sum =
...
int main(int argc, char* argv[]) {
    ...
    for (int t = 0; t < (int)TIMESTEPS; ++t) {
        ...
        #else
            stencil_3d_7pt_unrolled(curr, next);
        #endif
        std::swap(curr, next);
    }
    end = HR::now();
