...
//Use Padding for word_counter to remove false sharing
...
void print_counters() {
  for (int id = 0; id < MAX_THREADS; ++id) {
    std::cout << "Thread " << id << " counter: " << tracker.word_count[id].val
              << '\n';
  }
}
...
void* thread_runner(void* th_args) {
  ...
  while (getline(input_file, line)) {
    ...
    while ((pos = line.find(delimiter)) != std::string::npos) {
      ...
      // false sharing: on word count --removed as word_count is padded
      tracker.word_count[thread_id].val++;
      local_word_count++;
      // true sharing: on lock and total word variable... 
      //removed by keeping a local variable and updating it
