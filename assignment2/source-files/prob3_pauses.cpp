    ...
    void acquire(uint16_t tid) override {
        for (int i = 1; i < NUM_THREADS; i++) {
            ...
            for (int k = 0; k < NUM_THREADS; k++) {
                if (k == tid) continue;
                while (level[k].val >= i && victim[i].val == tid) thr_wait();
            }
        }
    }
...
public:
    void acquire(uint16_t tid) override {
        ...
        for (int k = 0; k < NUM_THREADS; k++) {
            if (k == tid) continue;
            while (choosing[k].val) thr_wait();
            while (ticket[k].val != 0 && (ticket[k].val < ticket[tid].val || (ticket[k].val == ticket[tid].val && k < tid))) thr_wait();
        }
    }
    void release(uint16_t tid) override {
        memory_barrier();
...
public:
    void acquire(uint16_t tid) override {
        while (atomic_lock_test_and_set((volatile int*)&flag)) thr_wait();
        memory_barrier();
    }
    void release(uint16_t tid) override {
        memory_barrier();
...
public:
    void acquire(uint16_t tid) override {
        ...
        while (serving_now != my_ticket) thr_wait();
        memory_barrier();
    }
    void release(uint16_t tid) override {
        memory_barrier();
...
public:
    void acquire(uint16_t tid) override {
        ...
        mySlot[tid] = slot;
        while (!flags[slot].flag) thr_wait();
